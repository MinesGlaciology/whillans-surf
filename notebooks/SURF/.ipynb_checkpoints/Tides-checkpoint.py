...
class Tide:
    ...
    def tidal_elevation(
        ...
        Chooses to use pyTMD map or pyTMD time_series functions based on relative
        lengths of lons and datetimes.

        lats and lons must be lists even if only one location.
        ...
        # Cull c to just the constituents we ask for
        if consts is not None:
            c = np.array(c)
            c = c[np.isin(c, consts)]
        
        # Use tidal_elevation_map() if more than one location
        if len(lons) > 1:
            tide_holder = []
        ...
        # Use tidal_elevation_time_series() if one location
        else:
